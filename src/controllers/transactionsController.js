...
const transfer = (req, res) => {
  ...
  const requiredFields = ['numero_conta_origem', 'numero_conta_destino', 'valor', 'senha']
  ...
  validOriginAccount = accounts.find((account) => {
    return account.numero === numero_conta_origem
  })

  validTargetAccount = accounts.find((account) => {
    return account.numero === numero_conta_destino
  })

  if (!validOriginAccount || !validTargetAccount) {
    return res.status(404).json({ mensagem: 'O número da conta informado não é válido.' })
  }

  const accountPassword = validOriginAccount.usuario.senha

  if (senha !== accountPassword) {
    return res.status(400).json({ mensagem: 'A senha informada está incorreta.' })
  }

  const accountBalance = validOriginAccount.saldo
  ...
  validOriginAccount.saldo -= Number(valor)
  validTargetAccount.saldo += Number(valor)
